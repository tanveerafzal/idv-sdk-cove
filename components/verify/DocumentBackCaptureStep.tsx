...
interface DocumentBackCaptureStepProps {
...
export default function DocumentBackCaptureStep({
  data,
  onNext,
  onBack,
  updateData,
}: DocumentBackCaptureStepProps) {
  const [capturedImage, setCapturedImage] = useState<string | null>(data.documentBackImage)
  ...
  useEffect(() => {
    if (data.retakeDocumentBack) {
      updateData({ retakeDocumentBack: false })
      startCamera()
    }
  }, [])
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        updateData({ documentBackImage: base64String })
      }
  ...
  const handleRetake = () => {
    ...
    updateData({ documentBackImage: null })
  }
    ...
    return (
      <IDCaptureOverlay
        ...
        onCapture={(imageData) => {
          ...
          updateData({ documentBackImage: imageData })
          stopCamera()
          onNext()
        }}
        onBack={stopCamera}
    ...
    <div className="flex flex-col h-full animate-fade-in">
      <div className="flex-1 space-y-6">
        <div className="w-full mb-4">
          <Image src="/mock-id.svg" alt="ID Document Back" width={400} height={240} className="w-full h-auto" />
        </div>

        <div className="space-y-2">
          <h2 className="text-l font-bold text-gray-900">
            Now take a photo of the back of your {getDocumentTypeLabel()}
          </h2>
        </div>
            ...
            <>
              <div className="space-y-3">
                <ul className="text-sm text-gray-600 space-y-2">
                  <li>• Make sure the barcode or magnetic stripe is visible</li>
